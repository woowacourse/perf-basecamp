...
`load more` 한 번의 비용은 **서로 성질이 다른 세 덩어리**이고, 절차 문서가 지시한 "응답 직후 1초" 한 구간으로 세면 셋이 뭉개진다.
...
절차 문서는 "GIPHY 응답 직후부터 1초"를 세라고 지시한다. 그 구간에는 **30~45ms짜리 커밋**과 **나머지 950ms의 이미지 수신**이 같이 들어가는데, 둘은 원인도 담당 TODO도 다르다. 그래서 아래처럼 나눠 셌다. 마지막 행이 절차 문서가 지시한 구간이며, 대조용으로 같이 싣는다.
...
- **절차 문서 §S5의 구간을 둘로 쪼갠다.** "응답 직후 1초"는 커밋(30~45ms)과 이미지 수신(950ms)을 섞는다. `Commit` 종료를 경계로 나눠 센다.
