...
- [v] Home 페이지에서 불러오는 스크립트 리소스에 gif 검색을 위한 giphy 모듈이 포함되어 있지 않아야 한다.
...
- [v] CDN을 적용하고, 한 번 요청한 리소스는 CDN 캐시에서 불러와야 한다.
- [v] GIPHY의 trending API를 Search 페이지에 들어올 때마다 새로 요청하지 않아야 한다.
...
- [v] 검색 결과 > 추가 로드시 추가된 목록만 렌더되어야 한다.
- [v] LayoutShift 없이 hover 애니메이션이 일어나야 한다.
