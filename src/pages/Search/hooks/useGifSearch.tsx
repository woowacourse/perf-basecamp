...
import { cacheOrFetch } from '../../../utils/cacheOrFetch';
  ...
  useEffect(() => {
    const fetch = async () => {
      if (status === SEARCH_STATUS.BEFORE_SEARCH) {
        const CACHE_KEY = 'GIF_TRENDING';
        const gifs: GifImageModel[] = await cacheOrFetch(CACHE_KEY, gifAPIService.getTrending);
