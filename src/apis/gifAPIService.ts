...
const cachedGifs: Record<string, unknown> = {};

const fetchGifs = <T>(cacheKey: string, apiCallFunc: () => T): T => {
  if (cachedGifs[cacheKey]) {
    return cachedGifs[cacheKey] as T;
  }

  const gifs = apiCallFunc();
  cachedGifs[cacheKey] = gifs;
  return gifs;
};
...
export const gifAPIService = {
  ...
  getTrending: async function (): Promise<GifImageModel[]> {
    try {
      const gifs: GifsResult = await fetchGifs('trending', () =>
